...
$stmt1 = $conn->prepare("
    ...
    WHERE e.status = 'archived' AND e.booking_type = 'Wedding'
");
if ($stmt1) {
    $stmt1->execute();
...
$stmt2 = $conn->prepare("
    ...
    WHERE e.status = 'archived' AND e.booking_type = 'Burial'
");
...
$stmt3 = $conn->prepare("
    ...
    WHERE e.status = 'archived' AND e.booking_type = 'Baptismal'
");
  ...
  <?php if (!empty($weddingResults)): ?>
    <?php foreach ($weddingResults as $row): ?>
      <div class="request-card">
        ...
        <a href="archived.admin.details.wedding.php?id=<?= urlencode($row['wedding_applications_id']) ?>" class="view-more-btn">View More</a>
      </div>
    <?php endforeach; ?>
  <?php else: ?>
    <p>No archived wedding applications found.</p>
  <?php endif; ?>
  ...
  <?php if (!empty($burialResults)): ?>
    <?php foreach ($burialResults as $row): ?>
      <div class="request-card">
        ...
        <p>Book Date: <?= htmlspecialchars($row['Book_Date']) ?></p>
        <p>Start time: <?= htmlspecialchars($row['Start_time']) ?></p> <br>
        <a href="archived.admin.details.burial.php?id=<?= urlencode($row['burial_requirements_id']) ?>" class="view-more-btn">View More</a>
      </div>
    <?php endforeach; ?>
  <?php else: ?>
    <p>No archived burial requests found.</p>
  <?php endif; ?>
  ...
  <?php if (!empty($baptismalResults)): ?>
    <?php foreach ($baptismalResults as $row): ?>
      <div class="request-card">
        ...
        <p>Book Date: <?= htmlspecialchars($row['Book_Date']) ?></p>
        <p>Start time: <?= htmlspecialchars($row['Start_time']) ?></p> <br>
        <a href="archived.admin.details.baptismal.php?id=<?= urlencode($row['baptismal_bookings_id']) ?>" class="view-more-btn">View More</a>
      </div>
    <?php endforeach; ?>
  <?php else: ?>
    <p>No archived baptismal requests found.</p>
  <?php endif; ?>
</div>
