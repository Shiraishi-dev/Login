...
// Update approved → ongoing if current time has reached start time
$updateOngoingQuery = "
    UPDATE event 
    SET status = 'ongoing' 
    WHERE status = 'approved' 
    AND CONCAT(Book_Date, ' ', Start_time) <= ?
    AND booking_type IN ('Wedding', 'Burial', 'Baptismal')
";
$stmtOngoing = $conn->prepare($updateOngoingQuery);
$stmtOngoing->bind_param('s', $currentDatetime);
$stmtOngoing->execute();
$stmtOngoing->close();

// Update ongoing → archived if 1 hour has passed after start time
$archiveQuery = "
    UPDATE event 
    SET status = 'archived' 
    WHERE status = 'ongoing' 
    AND DATE_ADD(CONCAT(Book_Date, ' ', Start_time), INTERVAL 1 HOUR) <= ?
    AND booking_type IN ('Wedding', 'Burial', 'Baptismal')
";
$stmtArchive = $conn->prepare($archiveQuery);
$stmtArchive->bind_param('s', $currentDatetime);
$stmtArchive->execute();
$stmtArchive->close();

// Wedding Applications (ongoing only)
$stmt1 = $conn->prepare("
    ...
    WHERE e.status = 'ongoing' AND e.booking_type = 'Wedding'
");
if ($stmt1) {
    $stmt1->execute();
...
$stmt2 = $conn->prepare("
    ...
    WHERE e.status = 'ongoing' AND e.booking_type = 'Burial'
");
...
// Baptismal Bookings (ongoing only)
$stmt3 = $conn->prepare("
    ...
    WHERE e.status = 'ongoing' AND e.booking_type = 'Baptismal'
");
...
<div class="client-requests">
  ...
  <?php if (!empty($weddingResults)): ?>
    ...
    <?php endforeach; ?>
  <?php else: ?>
    <p>No ongoing wedding applications found.</p>
  <?php endif; ?>
  ...
  <?php if (!empty($burialResults)): ?>
    <?php foreach ($burialResults as $row): ?>
      <div class="request-card">
        <h4>Deceased Name: <?= htmlspecialchars($row['deceased_name']) ?></h4> <br>
    ...
    <?php endforeach; ?>
  <?php else: ?>
    <p>No ongoing burial requests found.</p>
  <?php endif; ?>
  ...
  <?php if (!empty($baptismalResults)): ?>
    <?php foreach ($baptismalResults as $row): ?>
      <div class="request-card">
        <h4>Son/Daugther: <?= htmlspecialchars($row['child_first_name'] . ' ' . $row['child_middle_name']. ' '. $row['child_last_name']) ?></h4> <br>
    ...
    <?php endforeach; ?>
  <?php else: ?>
    <p>No ongoing baptismal requests found.</p>
  <?php endif; ?>
</div>
